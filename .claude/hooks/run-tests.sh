...
# 17b) prompt-router: auth & payments routes (JA + EN true positives)
out=$(printf '{"prompt":"Stripeで決済機能を実装したい"}' | bash "$H/prompt-router.sh")
printf '%s\n' "$out" | grep -q "stripe-payments" && ok "router routes JA payment prompt" || ng "router misses JA payment prompt"
out=$(printf '{"prompt":"ログイン機能と会員登録を追加"}' | bash "$H/prompt-router.sh")
printf '%s\n' "$out" | grep -q "auth-patterns" && ok "router routes JA auth prompt" || ng "router misses JA auth prompt"

# 17c) prompt-router: auth/payments false positives must not route
out=$(printf '{"prompt":"git checkout main and check the commit author"}' | bash "$H/prompt-router.sh")
printf '%s\n' "$out" | grep -qE "auth-patterns|stripe-payments" && ng "router false-positive on git checkout / author" || ok "router ignores git checkout / author"
out=$(printf '{"prompt":"add a striped background to the table"}' | bash "$H/prompt-router.sh")
printf '%s\n' "$out" | grep -q "stripe-payments" && ng "router false-positive on 'striped'" || ok "router ignores 'striped'"
